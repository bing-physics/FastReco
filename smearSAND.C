...
public:
    ...
    int nEntry;
    int rootrackerEntry;
    void GetEntry(int i){ iEntry=i; gEDepSimTree->GetEntry(i); rootrackerTree->GetEntry(i); }
    void ScanRackerTree(int i);
};
edepAna *ana;

void edepAna::ScanRackerTree(int i){
    ...
    else isHtarget=false;
    
   
    
}
void edepAna::loadfile(std::string inputfilename){
    inFile=new TFile(inputfilename.c_str());
    geo = (TGeoManager*) inFile->Get("EDepSimGeometry");
    
    gEDepSimTree = (TTree*) inFile->Get("EDepSimEvents");
    event=new TG4Event();
    gEDepSimTree->SetBranchAddress("Event",&event);    

    rootrackerTree=(TTree*) inFile->Get("DetSimPassThru/gRooTracker");
    brStdHepPdg    = rootrackerTree -> GetBranch ("StdHepPdg");
    brStdHepP4        = rootrackerTree -> GetBranch ("StdHepP4");
    ...
    nEntry=gEDepSimTree->GetEntries();
    rootrackerEntry=rootrackerTree->GetEntries();
    if(nEntry!=rootrackerEntry) {std::cout<<"----->----->not same entries"<<std::endl; std::exit(EXIT_FAILURE);}
...
void fill1Par(double recoPx, double recoPy, double recoPz, int trackid, double len, int nXhit, int nYhit, int smeartype){
  ...
  par.recoP4[1]=recoPy;
  par.recoP4[2]=recoPz;
  par.recoP4[3]=sqrt(recoPx*recoPx+recoPy*recoPy+recoPz*recoPz+m*m);
  par.trueP4[0]=event->Trajectories[trackid].InitialMomentum.X();
  par.trueP4[1]=event->Trajectories[trackid].InitialMomentum.Y();
  par.trueP4[2]=event->Trajectories[trackid].InitialMomentum.Z();
  par.trueP4[3]=event->Trajectories[trackid].InitialMomentum.E();
  //  par.trackid=trackid;
  par.parentid=event->Trajectories[trackid].ParentId;
  par.topid=findTopParent(trackid);
  par.toppdg=event->Trajectories[par.topid].PDGCode;
  par.smeartype=smeartype;
  par.length=len;
  par.lastECALlayer=-1;

  if(debug>=3) std::cout<<"fill --> pdg:"<<pdg<<" topid:"<<par.topid<<"toppdg:"<<par.toppdg<<std::endl;
  oneevent->particles[trackid]=par;
  //  std::cout<<"trackid:"<<trackid<<"  pdg:"<<pdg<<"  recoPx:"<<recoPx<<" recoPy:"<<recoPy<<"  recoPz:"<<recoPz<<std::endl;
  //  std::cout<<"par.trueP4[0]:"<<par.trueP4[0]<<" par.trueP4[1]:"<<par.trueP4[1]<<" par.trueP4[2]:"<<par.trueP4[2]<<std::endl;
}
/*
...
int findPrimaryId(int trackid){
  ...
  else { std::cout<<"findPrimaryId check!!!!, trackid:"<<trackid<<" parent:"<<event->Trajectories[parentId].Name<<" iEntry:"<<ana->iEntry<<std::endl; return parentId;}
}
...
bool smearPar_ecal(int trackid, int primaryId=-1){
  ...
  if(event->Trajectories[trackid].ParentId==-1 || primaryId==trackid ||  parentName=="pi0"){
    if(debug>=1) std::cout<<"didnot choose bruteforce since it is its own primaryId, trackid:"<<trackid<<" iEntry:"<<ana->iEntry<<std::endl;
  ...
  fill1Par(P3_smear.X(), P3_smear.Y(), P3_smear.Z(),  trackid, -999, -999, -999, ecalsmear);
  return true;
}
...
bool smearChargedPar_stt(int trackid){
  ...
  for( ;i<(ihit+nhit);i++){
    ...
    if(postPos.T()< prePos.T()) { // the time reversed hits are usually bad hits, break it
      ...
      if((i-ihit)*1.0<0.5*nhit && nhit>15) std::cout<<"thistrack has >15hits but break befor half,mayneed lookinto!!!nhit: "<<nhit<<" i-ihit:"<<i-ihit<<" trackid:"<<trackid<<" iEntry:"<<ana->iEntry<<std::endl;
      break;}
  ...
  */
  ...
  fill1Par(Px_smear*1000., Py_smear*1000., Pz_smear*1000., trackid, L*1000, nXhit, nYhit, sttsmear);
...
int findgammaPrimaryId(int trackid){
  ...
  if(event->Trajectories[parentId].ParentId==-1) { std::cout<<" check this gamma parent(also top):"<<event->Trajectories[parentId].Name<<" gid:"<<trackid<<" iEntry:"<<ana->iEntry<<std::endl;; return parentId;}
  ...
  std::cout<<"parent:"<<event->Trajectories[parentId].Name<<" grand:"<<event->Trajectories[grandid].Name<<" this need to check trackid:"<<trackid<<" ientry:"<<ana->iEntry<<std::endl;
...
void smearDaughters(int trackid){
  ...
  while(no){
    if(no->Traj->PDGCode>999999) ; // if pdgcode is too large, it will make dbpdg->GetParticle()->Charge() bad!
      ...
      std::cout<<"a 0 charge daughter particle try to smear, will smear daughters!!  Name:"<<no->Traj->Name<<" id:"<<no->Traj->TrackId<<" parentID:"<<trackid<<" ientry"<<ana->iEntry<<std::endl;
...
void smearPi0_external(int trackid){
  ...
  fill1Par(Px_smear*1000, Py_smear*1000, Pz_smear*1000 , trackid, -999, -999, -999, pi0extern);
...
bool smearN_byEquation(double &psmear, int trackid){  // only for antinumu events with 1 neutron produced 
  ...
  if(oneevent->particles.size()<1) return false;
  if(oneevent->particles.find(0)==oneevent->particles.end()) return false;
  if(abs(oneevent->particles[0].pdg)!=13 && abs(oneevent->particles[0].pdg)!=11)  {std::cout<<"^^^^^^^^^^^^^ fail neutron smearing by equation since no lepton reconstructed, iEntry: "<<ana->iEntry<<std::endl; return false;}
  
  const double mpr = dbpdg->GetParticle(2212)->Mass()*1000;
  const double mmu = dbpdg->GetParticle(oneevent->particles[0].pdg)->Mass()*1000; // actually it could be electron
  const double mn = dbpdg->GetParticle(2112)->Mass()*1000;

  TLorentzVector p4hadreco(0,0,0,0);
  TLorentzVector p4mureco(oneevent->particles[0].recoP4[0],oneevent->particles[0].recoP4[1],oneevent->particles[0].recoP4[2],oneevent->particles[0].recoP4[3]);
  for(auto par:oneevent->particles){
    //    if(iChannel==0) std::cout<<"i:"<<i<<" brPdg[i]:"<<brPdg[i]<<std::endl;
    if(par.first==0) continue;
    p4hadreco+=TLorentzVector(par.second.recoP4[0],par.second.recoP4[1],par.second.recoP4[2],par.second.recoP4[3]);
  ...
  if(oneevent->particles[0].pdg==13)  herr_E_equa_N->Fill((en-E)/E*100, ana->iChannel);
  if(oneevent->particles[0].pdg==13)  herr_p_equa_N->Fill((psmear-P)/P*100, ana->iChannel);
  ...
  TLorentzVector p4n(event->Trajectories[1].InitialMomentum);
    ...
    if((en-E)/E*100>1) std::cout<<"!!!!!!!!!!!!!!!!!!!!!!! too large check !!!!!!!!!! "<<ana->iEntry<<std::endl;
  }
  */
...
bool smearNeutron(int trackid){
    ...
    if(event->Trajectories[trackid].InitialMomentum.X()==0 && event->Trajectories[trackid].InitialMomentum.Y()==0 && event->Trajectories[trackid].InitialMomentum.Z()==0) {
	//	std::cout<<" a static neutron, skip"<<std::endl;
	return false;
    }
  ...
  //  std::cout<<"isSTTdetectable:"<<isSTTdetectable<<" isECALdetectable:"<<isECALdetectable<<std::endl;
...
void smearPar(int trackid, std::string name){
  ...
  if(name=="mu-" || name=="mu+" || name=="proton" || name=="pi+" || name=="pi-") smearChargedPar(trackid);
    ...
    std::cout<<"--->####################################### unknown par:"<<name<<"  iEntry:"<<ana->iEntry<<" trackid:"<<trackid<<std::endl;
  ...
  if(dbpdg->GetParticle(event->Trajectories[trackid].PDGCode)->Charge()==0 || event->Trajectories[trackid].ParentId!=-1) return;
  if(oneevent->particles.find(trackid)==oneevent->particles.end()) return;
  std::map<int, std::pair<int,double> > Id_npe_earliestT;
  bool touchEcal=findEvis_inecal(event, geo, ecalMap_prim2, trackid, Id_npe_earliestT);
  if(!touchEcal) return;
  int lastlayer=getLastLayer(Id_npe_earliestT);

  int iOuter=-1;
  int iInner=-1;
  getYokeSurfaceInfo2(event, trackid, iOuter,iInner);

  if(iInner>-1)
    lastlayer=9;
  if(iOuter>-1)
    lastlayer=10;
  oneevent->particles[trackid].lastECALlayer=lastlayer;
...
void summarizeEvent(){
  ...
  for(auto par: oneevent->particles){
    recoNuPx+=par.second.recoP4[0];
    recoNuPy+=par.second.recoP4[1];
    recoNuPz+=par.second.recoP4[2];
  }
  oneevent->nurecoP[0]=recoNuPx;
  oneevent->nurecoP[1]=recoNuPy;
  oneevent->nurecoP[2]=recoNuPz;
...
void init(std::string outputfilename, std::string inputfilename){
    outTreeF=new TFile(outputfilename.c_str(),"recreate");
    outTree =new TTree("edepsmear_anaevt_tree","edep smear anaevt tree");
    outTree->Branch("event", "anaevt", &oneevent);

    anaevtMeta evtMeta;    
    TTree *metaTree=new TTree("edepsmear_meta","meta data for edep smear anaevt tree");    
    metaTree->Branch("meta","anaevtMeta",&evtMeta);
    evtMeta.edepfullname=inputfilename;
    metaTree->Fill();
    metaTree->Write();
...
void getExternalResolutionFiles(){
...
int main(int argc, char *argv[]){
  ...
  if(argc<5) { std::cout<<"at least four arguments needed: cc or nc, smeartype(0: regular, 1:sttfv, 2:ecalfv) , input, output, [debug1/2/3],[startentry],[nentry]"<<std::endl;return 0;}
  debug=-1;
  std::string current(argv[1]);
  if(current=="cc") isCC=true;
  else if(current=="nc") isCC=false;
  else {std::cout<<"wrong input for current, cc or nc"<<std::endl; return 1;}
  int  smearRegion=std::atoi(argv[2]);
  std::string inputfilename(argv[3]);
  std::string outputfilename(argv[4]);
  
  std::cout<<"--------smearRegion:"<<smearRegion<<"   isCC:"<<isCC<<std::endl;
  int testStartEntry=-1;
  int testNEntry=-1;
  if(argc>=6){
    if(std::strcmp(argv[5],"debug1")==0) debug=1;
    else if(std::strcmp(argv[5],"debug2")==0) debug=2;
    else if(std::strcmp(argv[5],"debug3")==0) debug=3;
  }
  if(argc>=7) { testStartEntry=std::atoi(argv[6]);}
  if(argc==8) { testNEntry=std::atoi(argv[7]);}
  ...
  init(outputfilename, inputfilename);
  ana=new edepAna();
  ana->loadfile(inputfilename);
  ///////////////////////////////////////////////////
  ...
  int endplusEntry=(testNEntry!=-1)?(startEntry+testNEntry):ana->nEntry;
  for(int i=startEntry;i<endplusEntry;i++){
    ...
    else if(smearRegion==2){
	minSTThit=6;
	if(inSTT(vtx.X(),vtx.Y(),vtx.Z()) || abs(vtx.X())>1690 ) continue;	
    }
    

    oneevent=new anaevt();
    oneevent->ientry=i;    
    oneevent->target=ana->StdHepPdg[1];
    oneevent->nupdg=ana->StdHepPdg[0];
    oneevent->nutrueP[0]=ana->StdHepP4[0][0]*1000.;
    oneevent->nutrueP[1]=ana->StdHepP4[0][1]*1000.;                                                                       
    oneevent->nutrueP[2]=ana->StdHepP4[0][2]*1000.;                                                                  

    //   std::cout<<"oneevent->nupdg:"<<oneevent->nupdg<<std::endl;
